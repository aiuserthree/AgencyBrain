    ...
    const renderOptionSpecs = () => {
        ...
        if (undefinedOptions.length > 0) {
            ...
            const getOptionInfo = (opt) => {
                ...
                // 3. 세부 키워드 매핑 (1400+ 옵션 대응)
                const keywordMappings = [
                    // === 인증/로그인 ===
                    { keywords: ['소셜 로그인', '소셜로그인'], desc: '카카오/네이버/구글/애플 OAuth 연동 간편 로그인', ui: '[버튼그룹] 소셜 로그인', action: '소셜선택 → OAuth인증 → 토큰발급 → 로그인' },
                    { keywords: ['본인인증', '본인 인증'], desc: 'NICE/PASS 연동 휴대폰 본인인증, CI/DI 수신', ui: '[팝업] 본인인증 모듈', action: '인증요청 → 본인확인 → CI/DI저장 → 완료' },
                    { keywords: ['이메일 인증', '이메일인증'], desc: '가입 이메일로 인증 링크 발송, 클릭 시 인증 완료', ui: '[폼] 이메일 입력', action: '이메일입력 → 발송 → 링크클릭 → 인증완료' },
                    { keywords: ['sms 인증', 'sms인증', '문자 인증'], desc: '휴대폰 SMS 인증번호 발송 및 확인', ui: '[폼] SMS 인증', action: '번호입력 → 인증번호발송 → 입력 → 확인' },
                    { keywords: ['2단계 인증', '2단계인증', 'otp'], desc: 'SMS/앱 OTP 추가 보안 인증', ui: '[폼] OTP 입력', action: 'OTP요청 → 코드입력 → 검증 → 인증완료' },
                    { keywords: ['생체인증', '생체 인증', '지문', 'face id'], desc: '지문/Face ID 등 생체 정보 인증', ui: '[버튼] 생체인증', action: '생체인증요청 → 스캔 → 검증 → 로그인' },
                    { keywords: ['자동 로그인', '자동로그인'], desc: '로그인 상태 유지 설정 (최대 30일)', ui: '[체크박스] 자동 로그인', action: '체크 → 토큰저장 → 자동로그인 → 만료관리' },
                    { keywords: ['비밀번호 찾기', '비밀번호찾기'], desc: '이메일/SMS로 비밀번호 재설정 링크 발송', ui: '[폼] 비밀번호 찾기', action: '이메일입력 → 링크발송 → 재설정 → 완료' },
                    { keywords: ['아이디 찾기', '아이디찾기'], desc: '본인인증 후 가입된 아이디 확인', ui: '[폼] 아이디 찾기', action: '본인인증 → 아이디조회 → 마스킹표시 → 확인' },
                    { keywords: ['간편인증', '카카오인증', 'pass인증'], desc: '카카오/PASS 간편 본인인증', ui: '[버튼] 간편인증', action: '서비스선택 → 앱인증 → 결과수신 → 완료' },
                    { keywords: ['공동인증서', '공인인증'], desc: '공동인증서 기반 본인 확인', ui: '[팝업] 인증서 선택', action: '인증서선택 → 비밀번호 → 검증 → 인증완료' },
                    { keywords: ['신분증 ocr', '신분증ocr'], desc: 'AI 신분증 촬영 및 OCR 자동 인식', ui: '[카메라] 신분증 촬영', action: '촬영 → OCR인식 → 정보추출 → 검증' },
                    { keywords: ['로그인 기록', '로그인기록', '접속 기록'], desc: '로그인 이력 조회, 이상 접속 탐지', ui: '[리스트] 로그인 기록', action: '기록조회 → 기기확인 → 이상탐지 → 알림' },
                    { keywords: ['기기 등록', '기기등록'], desc: '신뢰할 수 있는 기기 등록/관리', ui: '[리스트] 등록 기기', action: '기기확인 → 등록 → 관리 → 삭제' },
                    { keywords: ['패턴', 'pin'], desc: '패턴/PIN 번호 설정 및 인증', ui: '[입력] 패턴/PIN', action: '패턴설정 → 확인입력 → 저장 → 인증' },
                    
                    // === 회원 정보 ===
                    { keywords: ['프로필 사진', '프로필사진'], desc: '프로필 이미지 업로드 및 크롭', ui: '[업로더] 프로필 사진', action: '사진선택 → 크롭 → 업로드 → 저장' },
                    { keywords: ['닉네임'], desc: '닉네임 설정 및 중복 확인', ui: '[입력필드] 닉네임', action: '입력 → 중복확인 → 저장' },
                    { keywords: ['자기소개'], desc: '자기소개 텍스트 입력 (최대 200자)', ui: '[텍스트영역] 자기소개', action: '입력 → 글자수확인 → 저장' },
                    { keywords: ['관심사 태그', '관심사태그'], desc: '관심사 태그 선택/입력, 맞춤 추천 반영', ui: '[태그] 관심사 선택', action: '태그선택 → 추가 → 저장 → 추천반영' },
                    { keywords: ['공개 설정', '공개설정'], desc: '프로필 공개 범위 설정 (전체/친구/비공개)', ui: '[라디오] 공개 범위', action: '범위선택 → 저장 → 적용' },
                    { keywords: ['차단 관리', '차단관리', '차단'], desc: '차단 사용자 관리, 차단 해제', ui: '[리스트] 차단 목록', action: '차단목록 → 선택 → 해제 → 확인' },
                    { keywords: ['회원정보 수정', '회원정보수정', '정보 수정'], desc: '개인정보 수정, 변경 이력 관리', ui: '[폼] 회원정보 수정', action: '정보조회 → 수정 → 인증 → 저장' },
                    { keywords: ['배송지 관리', '배송지관리'], desc: '배송지 목록 관리, 기본 배송지 설정', ui: '[리스트] 배송지 목록', action: '배송지조회 → 추가/수정 → 기본설정 → 저장' },
                    
                    // === 주문/결제 ===
                    { keywords: ['신용카드', '카드 결제', '카드결제'], desc: '신용/체크카드 결제, 카드사 선택', ui: '[폼] 카드 정보 입력', action: '카드선택 → 정보입력 → 결제 → 완료' },
                    { keywords: ['간편결제'], desc: '카카오페이/네이버페이/토스 간편결제', ui: '[버튼] 간편결제 선택', action: '서비스선택 → 앱인증 → 결제 → 완료' },
                    { keywords: ['계좌이체', '계좌 이체'], desc: '실시간 계좌이체 결제', ui: '[폼] 계좌 정보', action: '은행선택 → 계좌입력 → 인증 → 이체완료' },
                    { keywords: ['가상계좌', '가상 계좌'], desc: '가상계좌 발급, 입금 확인 알림', ui: '[정보] 가상계좌 안내', action: '계좌발급 → 입금 → 확인 → 주문처리' },
                    { keywords: ['휴대폰 결제', '휴대폰결제'], desc: '휴대폰 소액결제', ui: '[폼] 휴대폰 결제', action: '번호입력 → 인증 → 결제 → 완료' },
                    { keywords: ['포인트 결제', '포인트결제'], desc: '보유 포인트로 결제, 부분 사용', ui: '[입력] 포인트 사용', action: '포인트확인 → 사용금액입력 → 적용 → 결제' },
                    { keywords: ['복합 결제', '복합결제'], desc: '포인트+카드 등 복합 결제', ui: '[폼] 복합 결제', action: '결제수단조합 → 금액배분 → 결제 → 완료' },
                    { keywords: ['해외 결제', '해외결제'], desc: '해외 카드/페이팔 결제 지원', ui: '[폼] 해외 결제', action: '통화선택 → 결제수단 → 결제 → 환율적용' },
                    { keywords: ['예약 이체', '예약이체'], desc: '지정 일시에 자동 이체 예약', ui: '[폼] 예약 이체', action: '금액입력 → 일시설정 → 예약 → 실행' },
                    { keywords: ['자동 이체', '자동이체'], desc: '정기 자동 이체 설정', ui: '[설정] 자동 이체', action: '조건설정 → 계좌연결 → 등록 → 실행' },
                    
                    // === 배송 ===
                    { keywords: ['출고 전 취소', '출고전취소'], desc: '출고 전 주문 취소 처리', ui: '[버튼] 주문 취소', action: '취소요청 → 사유선택 → 확인 → 환불처리' },
                    { keywords: ['예약 배송', '예약배송'], desc: '원하는 날짜/시간 배송 예약', ui: '[캘린더] 배송일 선택', action: '날짜선택 → 시간선택 → 예약 → 확정' },
                    { keywords: ['새벽 배송', '새벽배송'], desc: '밤 주문 시 다음날 새벽 7시 전 도착', ui: '[배지] 새벽배송 표시', action: '새벽배송선택 → 주문 → 배송 → 새벽도착' },
                    { keywords: ['당일 배송', '당일배송'], desc: '오전 주문 시 당일 배송 완료', ui: '[배지] 당일배송 표시', action: '지역확인 → 당일배송선택 → 주문 → 당일도착' },
                    { keywords: ['해외 배송', '해외배송'], desc: '해외 배송 가능 국가, 관세 안내', ui: '[폼] 해외 배송지', action: '국가선택 → 배송비확인 → 주문 → 배송' },
                    { keywords: ['신선배송', '신선 배송'], desc: '신선식품 냉장/냉동 배송', ui: '[배지] 신선배송', action: '신선상품선택 → 쿨배송 → 배송추적 → 수령' },
                    { keywords: ['온도 관리', '온도관리'], desc: '콜드체인 실시간 온도 모니터링', ui: '[차트] 온도 추적', action: '배송시작 → 온도추적 → 이상감지 → 알림' },
                    { keywords: ['부재시 조치', '부재시조치'], desc: '부재 시 문앞/경비실/픽업 선택', ui: '[라디오] 부재시 조치', action: '조치선택 → 저장 → 배송시반영' },
                    
                    // === 반품/교환/환불 ===
                    { keywords: ['반품 신청', '반품신청'], desc: '상품 반품 접수, 회수 예약', ui: '[폼] 반품 신청', action: '상품선택 → 사유선택 → 회수예약 → 접수완료' },
                    { keywords: ['교환 신청', '교환신청'], desc: '상품 교환 접수, 동일/다른 상품 선택', ui: '[폼] 교환 신청', action: '상품선택 → 교환상품 → 회수예약 → 접수' },
                    { keywords: ['환불 처리', '환불처리', '환불'], desc: '결제 취소, 환불 금액 계산, 환불 진행', ui: '[정보] 환불 안내', action: '환불요청 → 금액확인 → 처리 → 입금' },
                    { keywords: ['반품 배송비', '반품배송비'], desc: '반품 귀책 사유별 배송비 안내', ui: '[정보] 배송비 안내', action: '사유확인 → 배송비계산 → 안내 → 적용' },
                    { keywords: ['불량품 처리', '불량품처리'], desc: '불량품 사진 등록, 무상 교환/환불', ui: '[폼] 불량 신고', action: '사진등록 → 불량유형 → 접수 → 처리' },
                    { keywords: ['부분 반품', '부분반품'], desc: '주문 내 일부 상품만 반품', ui: '[체크박스] 반품 상품 선택', action: '상품선택 → 반품신청 → 부분환불 → 완료' },
                ...
                // 4. 최종 fallback - 옵션명 자체를 분석해서 생성
                const optWords = opt.split(/[\s/()]+/).filter(w => w.length > 0);
                let desc, ui, action;
                
                // 마지막 단어 기반 UI 타입 결정
                const lastWord = optWords[optWords.length - 1] || opt;
                if (['입력', '작성', '등록'].some(k => opt.includes(k))) {
                    desc = `${opt} 정보를 입력하고 등록 처리`;
                    ui = `[폼] ${opt}`;
                    action = `정보입력 → 유효성검사 → ${opt} 처리 → 완료`;
                } else if (['조회', '검색', '목록'].some(k => opt.includes(k))) {
                    desc = `${opt} 데이터 조회 및 결과 표시`;
                    ui = `[리스트/테이블] ${opt}`;
                    action = `조건설정 → ${opt} 실행 → 결과표시 → 상세보기`;
                } else if (['선택', '설정', '변경'].some(k => opt.includes(k))) {
                    desc = `${opt} 옵션 선택 및 적용`;
                    ui = `[셀렉트/라디오] ${opt}`;
                    action = `옵션표시 → ${opt} 선택 → 저장 → 적용`;
                } else if (['확인', '보기', '상세'].some(k => opt.includes(k))) {
                    desc = `${opt} 상세 정보 확인`;
                    ui = `[카드/모달] ${opt}`;
                    action = `항목클릭 → ${opt} 표시 → 확인`;
                } else if (['버튼', '클릭'].some(k => opt.includes(k))) {
                    desc = `${opt} 실행 버튼`;
                    ui = `[버튼] ${opt}`;
                    action = `버튼클릭 → ${opt} 처리 → 결과`;
                } else if (['알림', '푸시', '메시지'].some(k => opt.includes(k))) {
                    desc = `${opt} 수신 및 관리`;
                    ui = `[알림] ${opt}`;
                    action = `알림수신 → ${opt} 확인 → 처리`;
                } else if (['결제', '구매', '비용'].some(k => opt.includes(k))) {
                    desc = `${opt} 결제 처리`;
                    ui = `[결제폼] ${opt}`;
                    action = `금액확인 → ${opt} 결제 → 완료`;
                } else if (['예약', '일정', '스케줄'].some(k => opt.includes(k))) {
                    desc = `${opt} 일정 선택 및 예약`;
                    ui = `[캘린더] ${opt}`;
                    action = `일시선택 → ${opt} 예약 → 확정`;
                } else if (['통계', '분석', '차트'].some(k => opt.includes(k))) {
                    desc = `${opt} 데이터 분석 및 시각화`;
                    ui = `[차트] ${opt}`;
                    action = `기간설정 → ${opt} 조회 → 차트표시`;
                } else if (['관리', '설정'].some(k => opt.includes(k))) {
                    desc = `${opt} 설정 및 관리`;
                    ui = `[설정] ${opt}`;
                    action = `현황조회 → ${opt} 설정 → 저장 → 적용`;
                } else if (['발급', '증명', '인증'].some(k => opt.includes(k))) {
                    desc = `${opt} 발급 및 인증 처리`;
                    ui = `[폼] ${opt}`;
                    action = `신청 → ${opt} 처리 → 발급 → 다운로드`;
                } else if (['연동', 'api', '연결'].some(k => opt.includes(k))) {
                    desc = `${opt} 외부 서비스 연동`;
                    ui = `[설정] ${opt}`;
                    action = `서비스선택 → ${opt} 인증 → 연동완료`;
                } else {
                    desc = `${funcName}의 ${opt} 기능 수행`;
                    ui = `[인터페이스] ${opt}`;
                    action = `${opt} 선택 → 처리 → 결과확인 → 완료`;
                }
