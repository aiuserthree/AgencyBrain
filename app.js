        ...
        if (undefinedOptions.length > 0) {
            ...
            const getOptionInfo = (opt) => {
                ...
                for (const [key, value] of Object.entries(optionDetailsMap)) {
                    if (optLower.includes(key.toLowerCase()) || key.toLowerCase().includes(optLower)) {
                        return value;
                    }
                }
                
                // 키워드 기반 기본값 생성
                let desc = `${funcName}의 ${opt} 기능 - `;
                let ui = '[UI요소] ';
                let action = '';
                
                if (optLower.includes('검색') || optLower.includes('조회')) {
                    desc += '조건 설정 및 결과 필터링, 정렬 기능 제공';
                    ui += '[검색바+필터] 검색 인터페이스';
                    action = '조건입력 → 검색실행 → 결과표시 → 필터/정렬';
                } else if (optLower.includes('등록') || optLower.includes('신청') || optLower.includes('작성')) {
                    desc += '필수 정보 입력, 유효성 검사, 등록 처리';
                    ui += '[폼] 등록/신청 양식';
                    action = '정보입력 → 유효성검사 → 등록 → 완료알림';
                } else if (optLower.includes('관리') || optLower.includes('설정')) {
                    desc += '설정 변경, 상태 관리, 이력 조회 기능';
                    ui += '[설정] 관리 화면';
                    action = '현황조회 → 설정변경 → 저장 → 적용';
                } else if (optLower.includes('결제') || optLower.includes('구매')) {
                    desc += '결제수단 선택, 결제 정보 입력, 결제 처리';
                    ui += '[폼] 결제 화면';
                    action = '결제수단선택 → 정보입력 → 결제 → 완료';
                } else if (optLower.includes('예약') || optLower.includes('신청')) {
                    desc += '일정 선택, 예약 정보 입력, 예약 확정';
                    ui += '[캘린더] 예약 화면';
                    action = '일정선택 → 정보입력 → 예약 → 확인알림';
                } else if (optLower.includes('알림') || optLower.includes('푸시')) {
                    desc += '알림 설정, 수신 관리, 알림 이력 조회';
                    ui += '[설정] 알림 관리';
                    action = '알림설정 → 조건지정 → 수신 → 확인';
                } else if (optLower.includes('정보') || optLower.includes('안내')) {
                    desc += '상세 정보 제공, 카테고리별 조회, 검색 기능';
                    ui += '[카드/리스트] 정보 표시';
                    action = '카테고리선택 → 정보조회 → 상세보기 → 저장/공유';
                } else {
                    desc += '사용자 편의성을 고려한 직관적인 UI/UX 제공';
                    ui += `[${opt}] 전용 인터페이스`;
                    action = `${opt} 선택/입력 → 처리 → 결과표시 → 완료`;
                }
